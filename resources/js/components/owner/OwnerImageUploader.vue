<template>
    <div class="mb-4">
        ...
        <div @dragover="onParentDragOver" @dragleave="onParentDragLeave" @drop="onParentDrop" @click="triggerFileInput"
            :class="{
        ...
        <div v-if="images.length" class="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            <div v-for="(image, index) in images" :key="image.id || image.file.name" draggable="true"
...
const onDragStart = (index, e) => {
    ...
    console.debug('[OwnerImageUploader] onDragStart index=', index);
};
...
const onDrop = (index, e) => {
    ...
    console.debug('[OwnerImageUploader] onDrop from=', from, 'to=', to);
};
...
const onParentDrop = (e) => {
    ...
    if (files.length) {
        e.preventDefault();
        handleDrop(e);
        console.debug('[OwnerImageUploader] onParentDrop: files dropped count=', files.length);
    }
};
...
const processFiles = (newFiles) => {
    ...
    const existingNewSize = images.value.reduce((acc, item) => acc + (item.file ? item.file.size : 0), 0);
    ...
    if ((existingNewSize + incomingTotalSize) > props.maxTotalFileSize) {
        toast.error(`The combined total size of selected images exceeds ${Math.round(props.maxTotalFileSize / 1024 / 1024)}MB. Please choose smaller images or upload in smaller batches.`);
        return;
    }
    ...
    const sizeFiltered = acceptedFiles.filter(file => {
        if (file.size > props.maxFileSize) {
            toast.error(`File ${file.name} is too large (max ${Math.round(props.maxFileSize / 1024 / 1024)}MB).`);
            return false;
        }
        return true;
    });
