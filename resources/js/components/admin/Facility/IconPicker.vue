<template>
  <div class="max-w-xl mx-auto p-2.5">
    ...
    <div class="mb-3 relative">
      ...
      />
      <i
        class="fas fa-search absolute right-3 top-1/2 transform -translate-y-1/2 text-blue-950"
      ></i>
    </div>
    ...
    <div v-else>
      ...
      <div
        v-if="totalPages > 1"
        class="flex justify-center mt-6 space-x-1 text-sm"
      >
        <button
...
const filteredIcons = computed(() => {
  ...
  if (!query) {
    return allIcons.value.slice(0, 100);
  }
...
const fetchIcons = async () => {
  ...
  try {
    ...
    const freeIcons = Object.entries(data).flatMap(([id, icon]) => {
      const validStyles = icon.styles?.filter((style) =>
        ['solid', 'regular', 'brands'].includes(style)
      );
      return (
        validStyles?.map((style) => ({
          ...
          name: icon.label,
          style,
          searchTerms: [id, icon.label, ...(icon.search?.terms || [])].map((term) =>
            term.toLowerCase()
          ),
        })) || []
      );
    ...
    error.value = e.error || 'Failed to load icons. Please try again later.';
  } finally {
