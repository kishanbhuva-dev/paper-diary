...
const containerClasses = computed(() => clsx(
    ...
    (!isValid.value && touched.value) ? 'border-red-500 ring-1 ring-red-100' :
...
<template>
    <div :class="['mb-4', widthClass]">
        <label v-if="label" :for="label" class="block text-sm font-semibold text-gray-700 mb-1">
        ...
        <div :class="containerClasses">
            ...
            <span v-if="prefix" class="text-sm text-slate-500 shrink-0 bg-gray-50 px-1 rounded">{{ prefix }}</span>

            <textarea v-if="multiline" :id="label" v-model="innerValue" :placeholder="placeholder" :rows="rows"
                :disabled="disabled" :maxlength="computedMaxLength"
                :class="[inputClasses, disabled ? 'cursor-not-allowed opacity-60' : '']" @blur="onBlur"
                @input="touched ? validateInput() : null"></textarea>

            <input v-else :id="label" v-model="innerValue" :type="isPasswordType && !showPassword ? 'password' : 'text'"
                :placeholder="placeholder" :disabled="disabled" :maxlength="computedMaxLength"
                :class="[inputClasses, disabled ? 'cursor-not-allowed opacity-60' : '']" @blur="onBlur"
                @input="touched ? validateInput() : null" />

            <span v-if="suffix" class="text-sm text-slate-500 shrink-0">{{ suffix }}</span>

            <button v-if="isPasswordType && !disabled" type="button" @click="showPassword = !showPassword"
        ...
        <div class="flex justify-between items-start mt-1 h-5">
            <span class="text-xs transition-colors duration-200 font-medium"
