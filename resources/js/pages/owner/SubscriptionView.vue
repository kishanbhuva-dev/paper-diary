<template>
  <main class="bg-[#f8fafc] min-h-screen font-sans antialiased text-slate-900">
    ...
    <div  class="p-4">
      <!-- Header Section -->
      <div class="mb-12">
        <h1 class="text-3xl font-bold tracking-tight text-slate-900 mb-2">Subscription Management</h1>
        <p class="text-slate-500">Manage your subscription, billing, and payment methods</p>
      </div>

      <!-- Subscription Status Card -->
      <div v-if="subscription" class="mb-12">
...
:class="[
            ...
            <div class="space-y-6">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="p-4 bg-slate-50 rounded-xl">
                  <div class="flex items-center gap-3 mb-2">
                    <Icon icon="heroicons:calendar-20-solid" class="w-5 h-5 text-slate-400" />
                ...
                <div class="p-4 bg-slate-50 rounded-xl">
                  <div class="flex items-center gap-3 mb-2">
                    <Icon icon="heroicons:credit-card-20-solid" class="w-5 h-5 text-slate-400" />
              ...
              <div class="p-4 bg-blue-50 rounded-xl border border-blue-100">
                <div class="flex items-center gap-3 mb-2">
                  <Icon icon="heroicons:information-circle-20-solid" class="w-5 h-5 text-blue-600" />
      ...
      <div v-if="recentSubscriptions.length > 0" class="mt-12">
        <div class="mb-8">
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              ...
              <tbody class="divide-y divide-slate-200">
                <tr v-for="(sub, index) in recentSubscriptions" :key="sub.id" class="hover:bg-slate-50 transition-colors">
                  <td class="px-6 py-4">
      ...
      <div v-else class="text-center py-20">
        <div class="max-w-md mx-auto">
          <div class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <Icon icon="heroicons:credit-card-20-solid" class="w-10 h-10 text-slate-400" />
          </div>
...
:class="[
            ...
            <div class="bg-blue-50 rounded-xl p-6">
              <h4 class="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                <Icon icon="heroicons:calendar-20-solid" class="w-5 h-5 text-blue-600" />
                Billing Period
              </h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="text-sm font-medium text-slate-600">Period Start</label>
                  <p class="text-slate-900 font-medium">{{ formatDate(selectedSubscription.current_period_start) }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-slate-600">Period End</label>
                  <p class="text-slate-900 font-medium">{{ formatDate(selectedSubscription.current_period_end) }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-slate-600">Created Date</label>
                  <p class="text-slate-900 font-medium">{{ formatDate(selectedSubscription.created_at) }}</p>
                </div>
                <div v-if="selectedSubscription.canceled_at">
                  <label class="text-sm font-medium text-slate-600">Canceled Date</label>
                  <p class="text-slate-900 font-medium">{{ formatDate(selectedSubscription.canceled_at) }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-amber-50 rounded-xl p-6">
              <h4 class="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                <Icon icon="heroicons:information-circle-20-solid" class="w-5 h-5 text-amber-600" />
            ...
            <div class="bg-slate-50 rounded-xl p-6">
              <h4 class="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                <Icon icon="heroicons:clipboard-document-list-20-solid" class="w-5 h-5 text-slate-600" />
                Additional Information
              </h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                <div v-if="selectedSubscription.ended_at">
                  ...
                  <p class="text-slate-900 font-medium">{{ formatDate(selectedSubscription.ended_at) }}</p>
                </div>
              </div>
            </div>
          </div>
...
const getNextBillingDate = () => {
  if (subscription.value?.next_billing_date && subscription.value.next_billing_date !== 'No billing date') {
...
const reactivateSubscription = async (sub) => {
  try {
    const response = await axios.post('/api/stripe/subscription/reactivate', {
