    ...
    <div v-else class="container mx-auto px-4 py-12 sm:py-20 max-w-7xl">
      <transition name="fade-down" appear>
        <div class="mx-auto max-w-3xl text-center mb-16" v-if="!selectedPlanDetails">
          ...
          <h1 class="text-4xl font-extrabold tracking-tight text-slate-900 sm:text-6xl mb-6">
            ...
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">without the limits.</span>
          </h1>
      ...
      >
        ...
        >
          ...
          <div class="flex-grow">
            ...
            <div class="mt-8 pt-8 border-t border-slate-50 space-y-4">
              <div class="flex items-start gap-3">
                ...
                <span class="text-sm text-slate-600">{{ plan.description || "Full access to all features" }}</span>
              </div>
              <div class="flex items-start gap-3">
                <div class="flex-shrink-0 w-5 h-5 rounded-full bg-emerald-50 flex items-center justify-center mt-0.5">
                  <Icon icon="heroicons:check-16-solid" class="w-3.5 h-3.5 text-emerald-600" />
          ...
          <button class="mt-10 w-full py-4 px-6 rounded-2xl bg-slate-50 text-slate-900 font-bold text-sm transition-all group-hover:bg-blue-600 group-hover:text-white group-hover:shadow-lg group-hover:shadow-blue-200">
            Choose Plan
          </button>
        </div>
      </div>
      ...
      <transition name="fade" mode="out-in">
        <div v-if="selectedPlanDetails" class="max-w-2xl mx-auto">
          <div class="bg-white rounded-[2rem] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.08)] border border-slate-100 overflow-hidden">
            ...
            <div class="mx-8 p-6 rounded-2xl bg-gradient-to-br from-indigo-600 via-blue-600 to-blue-500 text-white flex items-center justify-between mb-10 shadow-xl shadow-blue-100">
              <div class="text-left">
                <p class="text-blue-100 text-[10px] font-bold uppercase tracking-widest mb-1 opacity-80">Selected Plan</p>
                <h4 class="font-bold text-xl tracking-tight">{{ selectedPlanDetails?.nickname || selectedPlanDetails?.name }}</h4>
              </div>
              <div class="text-right">
                ...
                <span class="text-blue-100 text-[11px] font-semibold block capitalize opacity-90 mt-0.5">per {{ selectedPlanDetails?.display_interval }}</span>
              </div>
            </div>
            ...
            <form @submit.prevent="handleSubscriptionPayment" class="px-8 pb-12">
              <div class="space-y-6">
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <Icon icon="heroicons:user-20-solid" class="h-5 w-5 text-slate-300" />
                    </div>
                    <input
                ...
                <div class="space-y-2">
                  <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider ml-1 text-left">Email Address</label>
                  <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <Icon icon="heroicons:envelope-20-solid" class="h-5 w-5 text-slate-300" />
                    </div>
                    <input
                ...
                <div class="space-y-2">
                  <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider ml-1 text-left">Card Details</label>
                  <div id="card-element" class="px-5 py-4 bg-white border border-slate-200 rounded-2xl shadow-sm min-h-[50px]"></div>
                  <p id="card-errors" role="alert" class="text-xs text-rose-500 mt-2 font-medium text-left"></p>
                </div>
                ...
                <div class="flex items-center justify-center gap-6 pt-4 border-t border-slate-50">
                   <div class="flex items-center gap-2 grayscale opacity-50">
                      <Icon icon="logos:stripe" class="h-4" />
                   </div>
                   <div class="w-px h-4 bg-slate-200"></div>
                   <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                     <Icon icon="heroicons:shield-check-20-solid" class="w-4 h-4 text-emerald-500" />
                     SCA Secured Payment
                   </p>
                </div>
                ...
                >
                  <div class="flex items-center justify-center relative z-10">
                    <Icon v-if="isSubmitting" icon="svg-spinners:ring-resize" class="w-5 h-5 mr-3" />
                    <span>{{ isSubmitting ? 'Processing...' : `Confirm & Pay $${selectedPlanDetails?.display_amount}` }}</span>
                  </div>
...
const checkAndPrefillUserDetails = () => {
  try {
    ...
    if (user.role === 'owner') {
      guestDetails.value.fullName = [user.firstName, user.lastName].filter(Boolean).join(' ') || user.name || '';
...
.fade-down-enter-active { transition: all 0.6s ease-out; }
