...
const isDateValid = computed(() => {
  ...
  return (
    checkIn.isValid() && checkOut.isValid() && !checkIn.isBefore(today) && checkOut.isAfter(checkIn)
  );
});
...
const handleOutsideClick = (event) => {
  if (
    isEditing.value &&
    stayDetailsSection.value &&
    !stayDetailsSection.value.contains(event.target)
  ) {
    handleEditToggle();
  }
};
...
const refreshAvailability = async (slug) => {
  try {
    ...
    const match =
      res.data.data.find((r) => r.id.toString() === selectedResourceId.value?.toString()) ||
      res.data.data[0];
  ...
  try {
    const { paymentIntent, error } = await stripe.value.confirmPayment({
      ...
      confirmParams: {
        ...
        payment_method_data: {
          billing_details: { name: guestDetails.value.fullName, email: guestDetails.value.email },
        },
      },
    });
    if (error) {
...
const updateGuests = (t, v) => {
  if (t === 'adults') {
    guestDetails.value.adults = Math.max(1, guestDetails.value.adults + v);
  } else {
    guestDetails.value.children = Math.max(0, guestDetails.value.children + v);
  }
};
