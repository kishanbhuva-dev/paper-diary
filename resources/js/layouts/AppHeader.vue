<template>
  <header class="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
      ...
      <div class="flex justify-between items-center h-16 lg:h-20">
        <div class="flex items-center">
          <a href="/" class="flex items-center space-x-2 transition-opacity hover:opacity-90">
            <img src="/public/main_logo.png" alt="Paper Diary" class="h-8 lg:h-10 w-auto" />
          </a>
        </div>
        ...
        <nav class="hidden lg:flex items-center space-x-8">
          <template v-for="item in navigationMenu" :key="item.label">
            <router-link
              v-if="!item.isDropdown && (!item.role || (item.role === 'user' && isAuthenticated && user?.role === 'user'))"
              :to="item.to"
            ...
            <div class="hidden lg:block relative ml-6">
              ...
              >
                ...
                <div class="ml-3 text-left mr-2">
                  <p class="text-xs font-bold text-gray-900 leading-none mb-0.5">{{ user?.firstName }}</p>
                  <p class="text-[10px] text-gray-500 uppercase tracking-tighter">{{ user?.role }}</p>
                </div>
                <Icon icon="lucide:chevron-down" class="h-4 w-4 text-gray-400 group-hover:text-blue-600" />
              </button>
              ...
              >
                ...
                <div class="p-2">
                  <router-link
                    ...
                    @click="openDropdown = null"
                  >
                    <div class="w-8 h-8 rounded-lg bg-gray-50 group-hover:bg-blue-100 flex items-center justify-center mr-3 transition-colors">
                      <Icon :icon="link.icon" class="h-4 w-4 text-gray-500 group-hover:text-blue-600" />
                    </div>
    ...
    <div v-if="isMobileMenuOpen" class="lg:hidden">
      <div class="fixed inset-0 bg-black/20 backdrop-blur-sm z-40" @click="toggleMobileMenu"></div>
      <div class="fixed inset-y-0 right-0 w-[280px] bg-white shadow-2xl z-50 p-6 flex flex-col transform transition-transform duration-300">
        ...
        <nav class="flex-1 space-y-2">
          <template v-for="item in navigationMenu" :key="item.label">
            ...
            <div v-else-if="item.isDropdown">
              ...
              <div v-if="openDropdown === item.label" class="pl-4 mt-1 space-y-1">
                <a
                  v-for="subItem in item.dropdownItems"
                  :key="subItem.label"
                  :href="subItem.to"
        ...
        <div class="mt-auto pt-6 border-t border-gray-100">
          <template v-if="!isAuthenticated">
            <router-link to="/login" class="btn-primary w-full block text-center py-3 rounded-xl" @click="toggleMobileMenu">
              Login Now
            </router-link>
          </template>
          <template v-else>
            <div class="flex items-center mb-4 px-2">
              <div class="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold mr-3">{{ userInitials }}</div>
              <div>
