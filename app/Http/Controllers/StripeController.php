...
class StripeController extends Controller
{
    ...
    public function createSubscription(Request $request)
    {
        try {
            ...
            if (count($existingSubscriptions->data) > 0) {
                ...
                try {
                    $userSubscription = \App\Models\Subscription::where('stripe_id', $existingSub->id)->first();
                    if (!$userSubscription) {
                        $userSubscription = \App\Models\Subscription::create([
                            'user_id' => $user->id,
                            'type' => 'default',
                ...
                if ($invoice->status === 'open') {
                    ...
                    if ($invoice->payment_intent) {
                        ...
                        if ($paymentIntent->status === 'succeeded') {
                            $userSubscription = \App\Models\Subscription::where('stripe_id', $request->subscriptionId)->first();
                            if ($userSubscription) {
                                $userSubscription->stripe_status = 'active';
                                $userSubscription->save();
                            }
    ...
    public function cancelSubscription(Request $request)
    {
        try {
            ...
            // Update database
            $dbSubscription = \App\Models\Subscription::where('stripe_id', $subscription['database']['stripe_id'])->first();
            if ($dbSubscription) {
    ...
    public function reactivateSubscription(Request $request)
    {
        try {
            ...
            // Update database
            $dbSubscription = \App\Models\Subscription::where('stripe_id', $subscriptionId)->first();
            if ($dbSubscription) {
